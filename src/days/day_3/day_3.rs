...
pub fn day_3_main() -> Answer{
    ...
    for i in 1..line_len{
        let filtered = lines.iter().filter(|x| x.starts_with(&search_oxy)).map(|x| x.to_owned()).collect::<Vec<String>>();
        if filtered.len() == 1{
    ...
    for i in 1..line_len{
        let filtered = lines.iter().filter(|x| x.starts_with(&search_co2)).map(|x| x.to_owned()).collect::<Vec<String>>();
        if filtered.len() == 1{
    ...
    let oxy_vec = search_oxy.chars().map(|x| x.to_digit(10).unwrap()).collect::<Vec<u32>>();
    let co2_vec = search_co2.chars().map(|x| x.to_digit(10).unwrap()).collect::<Vec<u32>>();
    ...
    Answer{day:3, part_1:part_1.to_string(), part_2:part_2.to_string(), duration:duration}
}
...
// Lines to bits
fn lines_to_bits(lines:&Vec<String>) -> Vec<u32> {
        lines.iter().map(|line| line.chars().map(|chr| chr.to_digit(10).unwrap())).flatten().collect::<Vec<u32>>()
}

// Transpose array direction from lines to columns
fn bits_to_columns(bits:&Vec<u32>, line_len:usize) -> Vec<Vec<u32>>{
...
// Find find the highest frequency in each column
fn get_frequency(columns:&Vec<Vec<u32>>, line_len:usize, gte_flag:bool) -> Vec<u32>{
