...
pub fn day_11_main() -> Answer{
    ...
    while sync_pos == 0 || step_counter < 100{
        ...
        while checked < set_to_flash.len(){
            ...
            if checked_poses[[pos.0,pos.1]] != 1{
                ...
                num_map.slice_mut(s![((pos.0 - 1) as i32).max(0)..(pos.0 + 2).min(10) as i32,((pos.1 - 1) as i32).max(0)..(pos.1 + 2).min(10) as i32])
                       .iter_mut()
                       .for_each(|v| *v += 1);
    ...
    Answer{day:11, part_1:part_1.to_string(), part_2:part_2.to_string(), duration:duration}
}